...
import pplot
DEBUG=False
RunOne=True #if I just want to run one dump
RunNone=False #If I don't want to run any of it
DistPlots=0
dumpOne=43
Plots=1
WriteData=False
...
for simNum in simNums:
	...
	runName="relRecon2p_"
	if simNum==1:
		pathName="/scr_verus/wernerg/vrun/relRecon/plasmoids/m1-400-sig30-thd1p875-thbp01-res2-ppc32/"
	else:
	...
	numCells=1600
	simLabel=str(numCells)+"_"+str(simNum)
	totalDumps=80
	ypos=numCells/4
	yposT=numCells*3/4
	print "y positions:"
	...
	if numpy.logical_and(RunNone==False,WriteData==True):
	...
	AVG=False
	if RunNone==False:
	    for i in range(1,totalDumps):
		dump=i
		if RunOne:
		    print "running just one!"
		    dump=dumpOne
		print "StepNum= %d"%(dump)   
		filename=pathName+runName+'yeeB_'+str(dump)+".h5"
		print filename
		Az=egan.makeAz(filename)
